...
$.fn.dataTableExt.oPagination.listboxWithButtons = {
    ...
    "fnUpdate": function (oSettings, fnCallbackDraw) {
        ...
        if (iCurrentPage==1)
        {
            $('.paginate_button.previous').attr('disabled',true);

        }
        else{
            $('.paginate_button.previous').attr('disabled',false);
        }
        if (iCurrentPage==iPages)
        {
            $('.paginate_button.next').attr('disabled',true);

        }
        else{
            $('.paginate_button.next').attr('disabled',false);
        }
    }
